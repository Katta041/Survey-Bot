...
def transcribe_sarvam_batch_tn():
    ...
    # Track completion
    completed_jobs = set()
    output_dir = os.path.join(tn_download_dir, "sarvam_outputs_chunked_tn")
    ...
    output_csv = os.path.join(tn_download_dir, 'tn_transcribed_metadata_sarvam.csv')
    df.to_csv(output_csv, index=False)
    
    print(f"Saved {len(df)} records ({success_count} successful) to {output_csv}")

if __name__ == "__main__":
    transcribe_sarvam_batch_tn()
