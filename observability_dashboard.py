...
with st.sidebar:
    ...
    st.markdown("<div style='height:1px;background:#1f2937;margin:20px 0'></div>", unsafe_allow_html=True)
    st.markdown(f"<div style='font-size:0.65rem;color:#374151'>DB · <code style='color:#4b5563'>{os.path.basename(DB_PATH)}</code></div>", unsafe_allow_html=True)
    ...
    if llm.empty:
        ...
        with mc1:
            ...
            fig.update_layout(**PLOTLY_LAYOUT, height=260,
                yaxis=dict(title="Cost USD", gridcolor="#1f2937"),
                yaxis2=dict(title="Latency ms", overlaying="y", side="right", gridcolor="#1f2937"))
            st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False})
...
<div class="footer-bar">
  📡 API Intelligence Dashboard &nbsp;·&nbsp; DB: <code>{os.path.basename(DB_PATH)}</code>
